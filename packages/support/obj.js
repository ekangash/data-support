...
export const obj = {
   ...
   */
  isset(source, propName) {
    return (
      this.assert(source) &&
      source.hasOwnProperty(propName) &&
      typeof source[propName] !== "undefined"
    );
  },
  /**
  ...
  freeFromEmptiness(source) {
    return Object.fromEntries(
      Object.entries(source).filter(([prop, item]) => {
        if (this.assert(item)) {
   ...
   */
  get(source, names, defaultValue = undefined) {
